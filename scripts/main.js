...
let excluded_letters = ["a", "m"];
let alphabet_letters_filtered = alphabet_letters.filter(
  (letter) => !excluded_letters.includes(letter)
);

let el_alphabet_letters = document.getElementById("alphabet");
let el_excluded_letters = document.getElementById("excluded-letters");
let el_modal_loader = document.getElementById("modal");
let el_modal_picked_letter = document.getElementById("picked-letter");

el_alphabet_letters.innerHTML = alphabet_letters_filtered.join(" | ");
el_excluded_letters.innerHTML = excluded_letters.join(" | ");

let button = document.getElementById("sort");
let btn_reset = document.getElementById("btn-reset");

// TODO: Animação de letra não está resetando, só funciona a primeira vez
button.addEventListener("click", function () {
  if (el_modal_loader.classList.contains("d-none")) {
    el_modal_loader.classList.remove("d-none");
    setTimeout(function () {
      let picked_letter = sort();
      el_modal_loader.classList.add("d-none");
      document.getElementById("input-picked-letter").value = picked_letter;
      el_modal_picked_letter.classList.remove("d-none");
      excluded_letters.push(picked_letter);
      alphabet_letters_filtered = alphabet_letters.filter(
        (letter) => !excluded_letters.includes(letter)
      );
      animateLetter();
      el_alphabet_letters.innerHTML = alphabet_letters_filtered.join(" | ");
...
btn_reset.addEventListener("click", function () {
  el_modal_picked_letter.classList.add("d-none");
});

function sort() {
  let picked_letter =
    alphabet_letters_filtered[
      Math.floor(Math.random() * alphabet_letters_filtered.length)
    ];
...
var height,
  ...
  textPixels = [],
  input;
var colors = ["#fffb00", "#ff7300", "#ff00c8", "#002bff", "#7a00ff"];
...
function initInput() {
  input = document.getElementById("input-picked-letter");
